...
import { Smartphone, Lock, Shield, Zap, Download, RefreshCw, Database, Cpu, Usb, CheckCircle2, AlertCircle, Volume2, VolumeX } from 'lucide-react';
...
import { HuaweiBackground } from '../../components/Animations/ModuleBackgrounds';
  ...
  const handleServiceSelect = async (serviceId: ServiceType) => {
    ...
    try {
      ...
      if (data.found) {
        ...
        let targetMode: DeviceMode = 'Fastboot';
        let instr = data.exploit.manualSteps ? JSON.parse(data.exploit.manualSteps).join('\n') : '';
        let speechInstr = `Okay, you'll need to enter Fastboot mode. Connect the device while holding the volume down button.`;
          ...
          <div className="flex items-center justify-between z-10">
            <div className="flex items-center gap-4">
              <div className="w-12 h-12 rounded-2xl bg-stone-100 flex items-center justify-center">
                <Smartphone className="text-stone-950 w-6 h-6" />
              </div>
              <div>
                <h3 className="text-xl font-bold text-white">Huawei Auto-Unlock</h3>
              ...
              {flowState === 'idle' && (
                ...
                >
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="text-stone-400 max-w-sm mx-auto">Plug in your Huawei device via USB to begin auto-detection.</p>
                  </div>
                  <button
              ...
              {flowState === 'device_connected' && deviceIdentity && (
                ...
                >
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    {[
                      { id: 'FRP', label: 'FRP Bypass', icon: Shield },
                      { id: 'HUAWEI_ID', label: 'Huawei ID', icon: Lock },
                      { id: 'FLASH', label: 'Board Software', icon: Download },
                      { id: 'FACTORY', label: 'Factory Reset', icon: RefreshCw },
                    ].map(s => (
                      <button
                        key={s.id}
